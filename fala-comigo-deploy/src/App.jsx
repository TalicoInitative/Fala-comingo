...
useEffect(()=>{try{const raw=localStorage.getItem("fala-v6");if(raw){const p=JSON.parse(raw);
...
else if(!p.lastDate){p.lastDate=today;p.streak=1;}setProg(p);localStorage.setItem("fala-v6",JSON.stringify(p))}
...
try{localStorage.setItem("fala-v6",JSON.stringify(np))}catch{};return np})},3e4);return()=>clearInterval(iv)},[]);

const save=useCallback(p=>{setProg(p);try{localStorage.setItem("fala-v6",JSON.stringify(p))}catch{}},[]);
const addXP=useCallback((n)=>{setProg(p=>{const np={...p,xp:p.xp+n};try{localStorage.setItem("fala-v6",JSON.stringify(np))}catch{};return np});
...
try{localStorage.setItem("fala-v6",JSON.stringify(np))}catch{};return np})},[]);
