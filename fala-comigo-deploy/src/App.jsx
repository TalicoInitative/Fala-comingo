...
import { speakPT, setSlowMode, getSlowMode } from "./utils/audio.js";
...
export default function App() {
  ...
  const [slow, setSlow] = useState(() => { try { return localStorage.getItem("fala-slow") === "1" } catch { return false } });
  const toggleSlow = () => { const s = !slow; setSlow(s); setSlowMode(s); try { localStorage.setItem("fala-slow", s ? "1" : "0") } catch {} };
  useEffect(() => { setSlowMode(slow) }, []);
  ...
  const runSpeech = useCallback(opts => {
    ...
    let done = false;
    const cl = () => { if (micTmr.current) clearTimeout(micTmr.current); micTmr.current = null; recRef.current = null };
    r.onstart = () => onStart?.();
    r.onresult = e => { done = true; const t = e.results?.[0]?.[0]?.transcript || ""; cl(); t.trim() ? onResult?.(t.trim()) : onError?.("Didn't hear anything."); onEnd?.() };
    ...
    recRef.current = r;
    micTmr.current = setTimeout(() => { if (!done) { try { r.abort() } catch {}; cl(); onError?.("Too long."); onEnd?.() } }, 10000);
  ...
  // ── Send message ──
  const sendChat = async (voiceText) => {
    const t = (voiceText || chatIn).trim();
    if (!t || busy) return;
    setMsgs(p => [...p, { id: nid.current++, role: "u", text: t }]);
    setBusy(true); setChatIn(""); setErr(null);
    saveStat(s => ({ ...s, sent: s.sent + 1 }));
    try {
      const sc = scenario ? SCENARIOS.find(s => s.id === scenario) : null;
      const lv = level === "beginner"
        ? "BEGINNER: Very simple Portuguese (3-5 words). Explain everything in English. Basic words only."
        : level === "intermediate"
        ? "INTERMEDIATE: Natural conversation. Mix PT/EN. Introduce new vocab with explanations."
        : "ADVANCED: Mostly Portuguese including slang (mano, tipo, né, tá ligado). Complex grammar. Minimal English.";

      const sugPrompt = level !== "advanced"
        ? '\n"sug": provide array of 2-3 short Portuguese phrases the student could say next (as reply options). Keep them natural and appropriate to the conversation. Example: ["Sim, por favor!","Não, obrigado","Quanto custa?"]'
        : '\n"sug": null';

      const quizPrompt = quizMode
        ? `\nQUIZ MODE: You are testing the student. Give them a translation challenge, fill-in-the-blank, or ask "how do you say X in Portuguese?". After they answer, tell them if they're right or wrong, explain why, then give the next challenge. Mix grammar, vocabulary, and common phrases. Track difficulty — get harder as they succeed.`
        : "";

      const sys = `You are Bia, expert Brazilian Portuguese tutor from São Paulo. Warm, encouraging.
RESPOND ONLY raw JSON (no markdown, no backticks): {"pt":"Portuguese response","en":"English translation/explanation","tip":"grammar/culture tip or null","fix":"correction of student's Portuguese or null"${sugPrompt}}
${sc ? `SCENARIO: ${sc.prompt} Stay in character.` : "FREE CHAT."}${quizPrompt} ${lv}
CORRECTIONS: Always rewrite their FULL sentence correctly, then explain why. Example fix: "→ Eu tenho 25 anos (use 'ter' for age, not 'ser')". Common traps: ser/estar, gender (o problema=masc), age=ter not ser, gostar DE, bom vs bem, excitado=aroused not excited, puxar=pull not push.
PRONUNCIATION tips when relevant: D+i/e→dj, T+i/e→tch, R/rr→h, Lh→ly, Nh→ny, final M→nasal, ão→nasal diphthong.
Brazilian Portuguese ONLY. ${view === "voice" ? "VOICE MODE: Extra short (1-2 sentences)." : "Keep concise (1-3 sentences)."}`;
      ...
      for (const m of recent) {
        if (m.role === "u") {
          ...
          hist.push({ role: "assistant", content: JSON.stringify({ pt: m.pt, en: m.en, tip: m.tip, fix: m.fix, sug: m.sug || null }) });
        }
      }
      ...
      const r = await fetch("/api/chat", {
        ...
        signal: ctrl.signal,
        body: JSON.stringify({ model: "claude-sonnet-4-6", max_tokens: 600, system: sys, messages: hist })
      });
      clearTimeout(tmr);
      ...
      setErr(e?.name === "AbortError" ? "Took too long — try again." : "Network error — check connection.");
      setTimeout(() => setErr(null), 6000);
    } finally { setBusy(false) }
  };

  const startScenario = sc => { setScenario(sc.id); setQuizMode(false); setMsgs([{ id: nid.current++, role: "a", pt: sc.open, en: sc.openEN, tip: null, fix: null, sug: null }]) };
  const startQuiz = () => { setQuizMode(true); setScenario(null); setView("text"); setMsgs([]); sendChat("Quiz me! Start with something " + level + " level.") };
  const goHome = () => { setView("home"); setScenario(null); setQuizMode(false); setMsgs([]) };

  // ── Colors ──
  const T1 = dark ? "#e0e0e0" : "#1a1a1a", T2 = dark ? "#bbb" : "#555", T3 = dark ? "#888" : "#888", T4 = dark ? "#666" : "#999";
  const cbg = dark ? "rgba(20,30,20,.92)" : "rgba(240,248,240,.94)";
  const bdr = dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.05)";
  const glass = dark ? "rgba(20,30,20,.88)" : "rgba(232,245,233,.88)";
  const lvC = { beginner: "#4CAF50", intermediate: "#FF9800", advanced: "#E91E63" };

  // ── Bia message card (shared between text & voice) ──
  const BiaMsg = ({ m, isVoice }) => (
    <div style={{ background: glass, backdropFilter: "blur(16px)", border: `1px solid ${bdr}`, borderRadius: "20px 20px 20px 4px", boxShadow: `0 3px 14px ${dark ? "rgba(0,0,0,.15)" : "rgba(0,0,0,.04)"}`, overflow: "hidden" }}>
      ...
      <div style={{ padding: "4px 14px 8px", display: "flex", gap: 12, alignItems: "center" }}>
        <button onClick={() => speakPT(m.pt, () => setSpk(true), () => setSpk(false))} className="b" style={{ background: "none", fontSize: 12, color: isVoice ? "#2E7D32" : "#0B4A3E", padding: 0, fontWeight: 700 }}>🔊 Hear it</button>
        {slow && <span style={{ fontSize: 10, color: T4 }}>🐢 slow</span>}
      </div>
      {/* Suggested responses */}
      {m.sug && Array.isArray(m.sug) && m.sug.length > 0 && (
        <div style={{ padding: "2px 10px 10px", display: "flex", flexWrap: "wrap", gap: 6 }}>
          {m.sug.map((s, i) => (
            <button key={i} onClick={() => { if (!busy) sendChat(s) }} className="b"
              style={{ padding: "6px 12px", borderRadius: 14, background: dark ? "rgba(76,175,80,.1)" : "rgba(200,230,201,.7)", border: `1px solid ${dark ? "rgba(76,175,80,.2)" : "rgba(76,175,80,.25)"}`, fontSize: 11, color: dark ? "#81C784" : "#1B5E20", fontWeight: 600, fontFamily: "Georgia,serif" }}>
              {s}
            </button>
          ))}
        </div>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, background: "linear-gradient(160deg,#E8F5E9 0%,#C8E6C9 40%,#A5D6A7 70%,#E8F5E9 100%)", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ maxWidth: 400, width: "100%", animation: "pop .6s" }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          <div style={{ display: "inline-flex", animation: "float 4s infinite ease-in-out" }}>
            <svg width="72" height="52" viewBox="0 0 28 20" style={{ borderRadius: 8, boxShadow: "0 8px 32px rgba(0,0,0,.25)", animation: "glow 3s infinite" }}><rect width="28" height="20" fill="#009739"/><polygon points="14,2 26,10 14,18 2,10" fill="#FEDD00"/><circle cx="14" cy="10" r="4.5" fill="#002776"/></svg>
          </div>
          ...
          <div style={{ fontSize: 15, color: "#888", marginTop: 6, fontStyle: "italic" }}>Speak Brazilian Portuguese with AI</div>
        </div>
        <div style={{ background: "rgba(255,255,255,.9)", backdropFilter: "blur(20px)", borderRadius: 28, padding: "32px 28px", boxShadow: "0 16px 64px rgba(0,0,0,.1),0 0 0 1px rgba(255,255,255,.8)" }}>
          ...
          <button onClick={doLogin} style={{ width: "100%", padding: "16px", borderRadius: 16, border: "none", cursor: "pointer", background: "linear-gradient(135deg,#0B4A3E,#1B6B56)", color: "#fff", fontSize: 17, fontWeight: 800, fontFamily: "Georgia,serif", boxShadow: "0 8px 28px rgba(11,74,62,.35)", transition: "all .2s" }}>Start Learning →</button>
        </div>
      </div>
    </div>
  );
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: dark ? "#0a0f0a" : "linear-gradient(160deg,#E8F5E9 0%,#C8E6C9 30%,#A5D6A7 60%,#E8F5E9 100%)", overflow: "hidden", fontFamily: "system-ui,sans-serif", color: T1 }}>
      ...
      <div style={{ background: dark ? "rgba(15,15,15,.95)" : "rgba(255,255,255,.75)", backdropFilter: "blur(24px)", padding: "10px 20px", display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0, borderBottom: `1px solid ${bdr}`, boxShadow: `0 4px 24px ${dark ? "rgba(0,0,0,.3)" : "rgba(0,0,0,.06)"}` }}>
        <button onClick={goHome} className="b" style={{ display: "flex", alignItems: "center", gap: 8, background: "none", padding: 0 }}>
          ...
          <span style={{ fontSize: W ? 18 : 15, fontWeight: 800, fontFamily: "Georgia,serif", color: T1 }}>Fala Comigo</span>
        </button>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <span style={{ fontSize: 11, color: T4, fontWeight: 600 }}>💬{stats.sent} 🔥{stats.streak}</span>
          <button onClick={toggleSlow} className="b" style={{ background: slow ? (dark ? "rgba(76,175,80,.15)" : "rgba(76,175,80,.12)") : (dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.04)"), borderRadius: 12, padding: "5px 10px", fontSize: 13, color: slow ? "#4CAF50" : T3 }} title={slow ? "Slow speech ON" : "Normal speed"}>{slow ? "🐢" : "🐇"}</button>
        ...
        <div style={{ maxWidth: W ? 560 : 440, margin: "0 auto", width: "100%", padding: `0 ${W ? 20 : 16}px` }}>
          ...
          {view === "home" && <div style={{ padding: "28px 0", animation: "fi .5s" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, marginBottom: 28 }}>
              <div style={{ position: "relative" }}>
                ...
                <div style={{ position: "relative", width: 80, height: 80, borderRadius: "50%", background: cbg, backdropFilter: "blur(20px)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40, boxShadow: `0 8px 32px ${dark ? "rgba(0,0,0,.3)" : "rgba(0,0,0,.1)"}` }}>🤖</div>
              </div>
              <div style={{ fontSize: W ? 22 : 19, fontWeight: 800, fontFamily: "Georgia,serif" }}>Bia</div>
              <div style={{ fontSize: 13, color: T3, textAlign: "center", lineHeight: 1.6, maxWidth: 300 }}>Your AI Portuguese tutor. I correct mistakes, explain grammar, and speak back to you.</div>
            </div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[{ id: "beginner", icon: "🌱", name: "Beginner" }, { id: "intermediate", icon: "📚", name: "Intermediate" }, { id: "advanced", icon: "🔥", name: "Advanced" }].map(l =>
                  <button key={l.id} onClick={() => setLvl(l.id)} className="b" style={{ flex: 1, padding: "14px 8px", borderRadius: 18, textAlign: "center",
                    ...
                    <div style={{ fontSize: 20, marginBottom: 4 }}>{l.icon}</div>{l.name}
                  </button>
                )}
              </div>
            </div>

            {/* Mode cards — Text / Voice / Quiz */}
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
              <button onClick={() => { setView("text"); setQuizMode(false); if (!msgs.length) setMsgs([]) }} className="b" style={{ padding: "24px 16px", borderRadius: 24, textAlign: "center",
                ...
                <div style={{ fontSize: 11, color: T3, marginTop: 4 }}>Type in PT or EN</div>
              </button>
              <button onClick={() => { setView("voice"); setQuizMode(false); if (!msgs.length) setMsgs([]) }} className="b" style={{ padding: "24px 16px", borderRadius: 24, textAlign: "center",
                ...
                <div style={{ fontSize: 11, color: T3, marginTop: 4 }}>Speak & listen</div>
              </button>
            </div>

            {/* Quiz Me button */}
            <button onClick={startQuiz} className="b" style={{ width: "100%", padding: "18px 20px", borderRadius: 22, display: "flex", alignItems: "center", gap: 14, textAlign: "left", marginBottom: 12,
              background: dark ? "rgba(255,152,0,.08)" : "linear-gradient(135deg,rgba(255,243,224,.9),rgba(255,224,178,.7))", backdropFilter: "blur(16px)",
              border: `2px solid ${dark ? "rgba(255,152,0,.2)" : "rgba(255,152,0,.15)"}`, boxShadow: `0 4px 20px ${dark ? "rgba(255,152,0,.08)" : "rgba(255,152,0,.06)"}` }}>
              <div style={{ width: 48, height: 48, borderRadius: 14, background: "linear-gradient(135deg,#E65100,#FF9800)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, flexShrink: 0, boxShadow: "0 4px 14px rgba(230,81,0,.3)" }}>🧠</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 800, fontFamily: "Georgia,serif" }}>Quiz Me</div>
                <div style={{ fontSize: 12, color: T4, marginTop: 2 }}>Bia tests your Portuguese — translate, fill blanks, answer</div>
              </div>
              <span style={{ color: T4, fontSize: 18 }}>→</span>
            </button>
            ...
            <button onClick={() => setView("scenarios")} className="b" style={{ width: "100%", padding: "18px 20px", borderRadius: 22, display: "flex", alignItems: "center", gap: 14, textAlign: "left",
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 800, fontFamily: "Georgia,serif" }}>Practice Scenarios</div>
                <div style={{ fontSize: 12, color: T4, marginTop: 2 }}>{SCENARIOS.length} real-world situations</div>
              </div>
              <span style={{ color: T4, fontSize: 18 }}>→</span>
            </button>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 20, justifyContent: "center" }}>
              {["Oi Bia! 😊", "How do I say 'thank you'?", "Teach me numbers", "What's ser vs estar?"].map((q, i) =>
                <button key={i} onClick={() => { setView("text"); sendChat(q) }} className="b"
                  style={{ padding: "10px 16px", borderRadius: 20, background: glass, backdropFilter: "blur(12px)", border: `1px solid ${bdr}`, fontSize: 13, color: T2, fontFamily: "Georgia,serif", animation: `fi .4s ${i * .06}s both` }}>
                  {q}
                </button>
              )}
            </div>
          ...
          {view === "scenarios" && <div style={{ padding: "20px 0", animation: "fi .4s" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setView("home")} className="b" style={{ fontSize: 13, color: T3, background: "none" }}>← Back</button>
            </div>
          ...
          {view === "text" && <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 130px)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0 8px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 16 }}>{quizMode ? "🧠" : scenario ? SCENARIOS.find(s => s.id === scenario)?.icon : "⌨️"}</span>
                <span style={{ fontSize: 14, fontWeight: 800, fontFamily: "Georgia,serif" }}>{quizMode ? "Quiz Mode" : scenario ? SCENARIOS.find(s => s.id === scenario)?.name : "Text Chat"}</span>
                <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 8, background: `${lvC[level]}18`, color: lvC[level], fontWeight: 700 }}>{level}</span>
              </div>
              <button onClick={goHome} className="b" style={{ fontSize: 12, color: T3, background: "none" }}>← End</button>
            </div>

            {msgs.length === 0 && !busy && <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
              <div style={{ textAlign: "center", color: T3 }}>
                <div style={{ fontSize: 40, marginBottom: 8 }}>{quizMode ? "🧠" : "⌨️"}</div>
                <div style={{ fontSize: 14 }}>{quizMode ? "Get ready — Bia is preparing your quiz..." : "Type something in Portuguese or English to start!"}</div>
              </div>
            </div>}
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, paddingBottom: 16 }}>
              {msgs.map(m => <div key={m.id} style={{ alignSelf: m.role === "u" ? "flex-end" : "flex-start", maxWidth: m.role === "u" ? "78%" : "86%", animation: "fi .25s" }}>
                {m.role === "u"
                  ? <div style={{ background: "linear-gradient(135deg,#0B4A3E,#1B6B56)", color: "#fff", padding: "12px 16px", borderRadius: "20px 20px 4px 20px", fontSize: 15, lineHeight: 1.5, boxShadow: "0 4px 16px rgba(11,74,62,.25)" }}>{m.text}</div>
                  : <BiaMsg m={m} isVoice={false} />
                }
              </div>)}
          ...
          {view === "voice" && <div style={{ display: "flex", flexDirection: "column", minHeight: "calc(100vh - 130px)" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <button onClick={() => { if (busy || voiceListening) return; runSpeech({ lang: voiceLang, onStart: () => setVoiceListening(true), onResult: t => { setVoiceListening(false); sendChat(t) }, onError: e => { setVoiceListening(false); setErr(e); setTimeout(() => setErr(null), 5000) }, onEnd: () => setVoiceListening(false) }) }} className="b"
                  style={{ width: 100, height: 100, borderRadius: "50%", background: voiceListening ? "linear-gradient(135deg,#E53935,#C62828)" : "linear-gradient(135deg,#C2185B,#E91E63)", color: "#fff", fontSize: 36, display: "flex", alignItems: "center", justifyContent: "center",
                    boxShadow: voiceListening ? "0 0 0 6px rgba(233,30,99,.2),0 8px 32px rgba(233,30,99,.4)" : "0 8px 32px rgba(233,30,99,.3)", animation: voiceListening ? "mic 1.5s infinite" : "" }}>🎙️</button>
              </div>
            ...
            {/* Transcript when conversation started */}
            {msgs.length > 0 && <>
              <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10, paddingBottom: 16 }}>
                {msgs.map(m => <div key={m.id} style={{ alignSelf: m.role === "u" ? "flex-end" : "flex-start", maxWidth: m.role === "u" ? "78%" : "86%", animation: "fi .25s" }}>
                  {m.role === "u"
                    ? <div style={{ background: "linear-gradient(135deg,#C2185B,#E91E63)", color: "#fff", padding: "12px 16px", borderRadius: "20px 20px 4px 20px", fontSize: 15, lineHeight: 1.5, boxShadow: "0 4px 16px rgba(233,30,99,.25)" }}>{m.text}</div>
                    : <BiaMsg m={m} isVoice={true} />
                  }
                </div>)}
                {busy && <div style={{ display: "flex", gap: 6, padding: 12, alignSelf: "flex-start" }}>{[0, .15, .3].map(d => <div key={d} style={{ width: 7, height: 7, borderRadius: "50%", background: T4, animation: `pulse 1s ${d}s infinite` }} />)}</div>}
                {err && <div style={{ padding: "10px 14px", background: "rgba(198,40,40,.06)", border: "1px solid rgba(198,40,40,.12)", borderRadius: 12, fontSize: 13, color: "#C62828" }}>{err}</div>}
      ...
      {view === "text" && <div style={{ background: dark ? "rgba(15,15,15,.95)" : "rgba(255,255,255,.85)", backdropFilter: "blur(20px)", borderTop: `1px solid ${bdr}`, padding: "10px 16px", flexShrink: 0 }}>
        <div style={{ maxWidth: W ? 560 : 440, margin: "0 auto", display: "flex", gap: 8, alignItems: "center" }}>
          <input value={chatIn} onChange={e => setChatIn(e.target.value)} onKeyDown={e => { if (e.key === "Enter") sendChat() }}
            placeholder={quizMode ? "Type your answer..." : "Type in Portuguese or English..."} style={{ flex: 1, padding: "13px 16px", borderRadius: 16, border: `1.5px solid ${dark ? "rgba(255,255,255,.08)" : "rgba(0,0,0,.06)"}`, fontSize: 15, background: dark ? "rgba(40,40,40,.8)" : "rgba(255,255,255,.9)", color: T1, outline: "none", backdropFilter: "blur(12px)" }} />
